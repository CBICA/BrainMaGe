...
def pad_image(image):
    ...
    if image.shape[0] <= 240:
        ...
        padded_image = np.pad(padded_image, ((pad_x1, pad_x2), (0, 0), (0, 0)),
    ...
    if image.shape[1] <= 240:
        ...
        padded_image = np.pad(padded_image, ((0, 0), (pad_y1, pad_y2), (0, 0)),
                              mode='constant', constant_values=0)

    # Padding on Z axes
    if image.shape[2] <= 160:
        pad_z2 = 160 - image.shape[2]
        padded_image = np.pad(padded_image, ((0, 0), (0, 0), (pad_z2, 0)),
                              mode='constant', constant_values=0)
