...
def infer_ma(cfg, device, save_brain, weights):
    ...
    for patient in tqdm.tqdm(test_df.values):
        ...
        new_image = resize(
            image_data, new_shape, order=3, mode="edge", cval=0, anti_aliasing=False
        )
        new_affine = np.eye(4)
        ...
        for i in range(3):
            for j in range(3):
                if old_affine[i, j] != 0:
                    ...
                    if old_affine[i, j] <= 0:
                        new_affine[i, j] = -1 * (
                            old_affine[i, j] * (1 / old_affine[i, j])
                        )
    ...
    for patient in tqdm.tqdm(test_df.values):
        ...
        with torch.no_grad():
            ...
            to_save_final = postprocess_prediction(to_save_final).astype(np.uint8)
    ...
    print(
        'Please check the %s folder for the intermediate outputs if you"+\
