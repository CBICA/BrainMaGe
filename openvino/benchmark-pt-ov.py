    ...
    with torch.no_grad():
        for i, row in tqdm(dataset_df.iterrows()):
            ...
            try:
                ...
                pt_stat = [i, sub_id, pt_hdorff_score, i_end - i_start, p_end - p_start]
...
def bench_ov_fp32():
    ...
    for i, row in tqdm(dataset_df.iterrows()):
        ...
        try:
            ...
            ov_stat = [i, sub_id, ov_hdorff_score, i_end - i_start, p_end - p_start]
    ...
    print(f"\n OpenVINO FP32 Hausdorff Distance Mean: {ov_stats_df[:][2].mean():.5f}")
    print(
        f"OpenVINO FP32 Total Inf Time: {ov_stats_df[:][3].sum():.2f} sec, Mean: {ov_stats_df[:][3].mean():.2f}"
    )
...
print(f"\n Mean Hausdorff Distance Scores for {dataset_df.shape[0]} images")
print(
    f"PyTorch Hausdorff Distance Mean +/- STD: {pt_stats_df[:][2].mean():.5f} +/- {pt_stats_df[:][2].std():.1f}"
)
print(
    f"OpenVINO FP32 Hausdorff Distance Mean +/- STD: {ov_stats_df[:][2].mean():.5f} +/- {ov_stats_df[:][2].std():.1f}"
)
print(
    f"OpenVINO INT8 Hausdorff Distance Mean +/- STD: {ov_int8_stats_df[:][2].mean():.5f} +/- {ov_int8_stats_df[:][2].std():.1f}"
)
...
print()
print(
    f"\n Accuracy Hausdorff Distance difference with OV FP32 {dice_diff_pt_ov.sum():.6f}"
)
# print(dice_diff_pt_ov.value_counts())
print(
    f" Accuracy Hausdorff Distance difference with OV INT8 {dice_diff_pt_ov_int8.sum():.6f}"
)
...
print(
    f"PyTorch: {pt_total_inf_time:.2f} , Mean +/- STD: {pt_stats_df[:][3].mean():.2f} +/- {pt_stats_df[:][3].std():.2f} "
)
print(
    f"OpenVINO FP32: {ov_total_inf_time:.2f} , Mean +/- STD: {ov_stats_df[:][3].mean():.2f} +/- {ov_stats_df[:][3].std():.2f} "
)
print(
    f"OpenVINO INT8: {ov_int8_total_inf_time:.2f} , Mean +/- STD: {ov_int8_stats_df[:][3].mean():.2f} +/- {ov_int8_stats_df[:][3].std():.2f} "
)
...
print(
    f"Speedup with OpenVINO INT8 for {pt_stats_df.shape[0]} images: {speedup_int8:.1f}x"
)
